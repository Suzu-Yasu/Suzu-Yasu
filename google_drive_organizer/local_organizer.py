...
def organize_inbox(
    ...
    inbox = root / INBOX_DIR_NAME

    if not inbox.exists():
        logger.error("Inbox フォルダが見つかりません: %s", inbox)
        sys.exit(1)
...
def watch_inbox(root: Path, use_ocr: bool = False) -> None:
    """
    ...
    class InboxHandler(FileSystemEventHandler):
        def on_created(self, event):
            ...
            # 単一ファイルを整理
            _process_single_file(filepath, root, use_ocr=use_ocr)
...
def _detect_drive_root() -> Optional[Path]:
    ...
    for p in candidates:
        if p.exists() and (p / INBOX_DIR_NAME).exists():
            return p
    return None
...
def main() -> None:
    ...
    if args.watch:
        logger.info("=== 監視モードで起動: %s/%s ===", root, INBOX_DIR_NAME)
        watch_inbox(root, use_ocr=args.ocr)
    else:
        stats = organize_inbox(root, dry_run=args.dry_run, use_ocr=args.ocr)
        print(stats.summary())
